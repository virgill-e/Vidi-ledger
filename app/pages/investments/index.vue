...
const handleExport = (format: 'csv' | 'pdf') => {
  const headers = ['Date', 'Actif', 'Type', 'Quantité', 'Montant', 'Note'];
  ...
  const title = `Rapport d'investissements - ${timeFilterLabel.value}`;
  const filename = `investissements_${timeFilter.value}_${new Date().toISOString().split('T')[0]}`;
